...
async def quit_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not bot_user:
        await update.message.reply_text('未绑定,请先绑定\n使用 /bind 命令绑定你的订阅（仅限私聊）', reply_markup=reply_markup)
        return START_ROUTES
...
async def start_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f'请发送`🎰`或`🎲`或`🏀`或`⚽`或`🎳`或`🔫`表情（单击此处可复制表情），可以连续发送\n当前赔率:\n🎰1赔{config.TIGER.rate} 🏀1赔{config.BASKETBALL.rate}\n⚽1赔{config.FOOTBALL.rate} 🎯1赔{config.BULLSEYE.rate}\n🎳1赔{config.BOWLING.rate} 🔫1赔{replace_dot(config.ROULETTE.rate)}\n发送"不玩了"退出赌博模式\n请选择下注流量或自定义：(不包含\🔫)')
    await query.edit_message_text(
